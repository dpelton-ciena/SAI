...
sub WriteHeaderHeader
{
    ...
    {
        $longest_enum = $enum if not defined $longest_enum or length($enum) > length($longest_enum);
    }

    my $primitive_buffer_size = defined $longest_enum ? length($longest_enum) + 1 : 1;

    WriteHeader "#ifndef __SAI_METADATA_H__";
    WriteHeader "#define __SAI_METADATA_H__";
    WriteHeader "#define PRIMITIVE_BUFFER_SIZE $primitive_buffer_size";
